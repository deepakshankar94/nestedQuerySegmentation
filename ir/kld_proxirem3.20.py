...
cf=pickle.load(fp1)
fp1.close()
f=open('tfmnewhuman.txt','rb')
tfmstore=pickle.load(f)
f.close()
#tfmstore={}
#f=open('tfmexp.txt','rb')
#tfmstore2=pickle.load(f)
#f.close()
#tfmstore.update(tfmstore2)
f=open("stops.txt","r")
stopworden=f.read()
...
for query in queries[:250]:
	...
	for file in files:
		...
		form[txt]=f3
		fpc=open('./expansions/'+str(qno)+'_h1.txt')
		...
		proxicpe[txt]=p1
		p1=0
		fpc=open('./expansions/'+str(qno)+'_h3.txt')
		correct=fpc.readlines()
		fpc.close()
		#print 'for cpes'
		for wor in correct:
