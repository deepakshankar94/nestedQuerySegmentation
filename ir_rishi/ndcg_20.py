...
files=sorted(files)
qno=251                #LOOK HERE CHANGE THIS
count=0
methods=['nestedr']
fpmiss=open('missing_ndcg_20.txt','w')
fmain=open('kldall_20.txt','w')
...
for fil in files[250:]:
	...
	for method in methods:
		fp=open('results/query_'+str(qno)+'_result_'+method+'_top_20.txt')
		methvals=fp.readlines()[1:]
		fp.close()
		val[method]=dcgcal(methvals,filerank,fil,method)/idcgcal(filerank)
	fp=open("ndcg/query_"+str(qno)+"_ndcg@20.txt",'w')
	for method in methods:
