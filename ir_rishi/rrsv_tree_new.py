...
for num in range(250,500):
	...
	rrsvfil={}
	fp=open('./kldValues/query_'+str(qno)+'_result_kld_top_50.txt')
	kldFiles=fp.readlines()[1:]
	fp.close()
	for i in range(len(kldFiles)):
	    kldValues[kldFiles[i].split()[0]]=kldFiles[i].split()[1]
	coun1=1
	...
	for i in range(len(files)):
		filc=files[i].split()[0]
		sav[filc]=float(kldValues[filc.split('C')[0]])+alpha*flis[filc]
